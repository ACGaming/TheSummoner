...
public class ItemSummonerOrb extends Item
{
        ...
        if (summonedEntity != null)
        {
            ...
            if (!worldIn.isRemote)
            {
                ...
                if (EntityUtil.getCapability(summonedEntity, CapabilitySummonableEntity.SUMMONABLE_ENTITY_CAPABILITY, null) != null)
                {
                    ...
                    MMMessageRegistry.getNetwork().sendToAllAround(new MessageSummonable(summonedEntity.getUniqueID().toString(), summonable.getSummonerId().toString(), summonable.isFollowing(), summonable.getTimeLimit()), new TargetPoint(player.dimension, summonedEntity.posX, summonedEntity.posY, summonedEntity.posZ, 255D));
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
    {
        ...
        {
            nbttagcompound.getString("EntityName");
            if (!nbttagcompound.getString("EntityName").isEmpty())
            {
                tooltip.add("Name: " + nbttagcompound.getString("EntityName"));
                ResourceLocation registryName = new ResourceLocation(nbttagcompound.getString("RegistryNameDomain"), nbttagcompound.getString("RegistryNamePath"));
                tooltip.add("Type: " + registryName);
            }
            else
            {
                tooltip.add("empty");
