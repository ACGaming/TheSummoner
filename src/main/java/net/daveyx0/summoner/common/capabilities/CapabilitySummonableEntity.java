...
public class CapabilitySummonableEntity
{
    ...
    public static class EventHandler
    {
        ...
        public static void EntityLivingDeathEvent(LivingDeathEvent event)
        {
            if (isEntitySuitableForSummon(event.getEntityLiving()))
            {
                ...
                if (spiritCanBeCaptured(entity, summonable, event.getSource()) || summonable != null && summonable.isSummonedEntity())
                {
                    ...
                    TheSummonerConfig.getMobBlackList();
        ...
        {
            if (!event.getEntityPlayer().getEntityWorld().isRemote && isEntitySuitableForSummon(event.getTarget()))
            {
                ...
                {
                    MMMessageRegistry.getNetwork().sendToAllAround(new MessageSummonable(event.getTarget().getUniqueID().toString(), summonable.getSummonerId().toString(), summonable.isFollowing(), summonable.getTimeLimit()), new TargetPoint(event.getEntityPlayer().dimension, event.getTarget().posX, event.getTarget().posY, event.getTarget().posZ, 255D));
                }
            }
        }
            ...
            if (isEntitySuitableForSummon(event.getTarget()) && event.getHand() == EnumHand.MAIN_HAND)
            {
                ...
                if (summonable != null && (summonable.isSummonedEntity() && summonable.getSummoner(entity) == event.getEntityPlayer() || !summonable.isSummonedEntity() && event.getEntityPlayer().isCreative()) && event.getEntityPlayer().getHeldItem(event.getHand()).getItem() instanceof ItemSummonerOrb && event.getHand() == EnumHand.MAIN_HAND)
                {
                    ...
                    if (event.getEntityPlayer().getHeldItem(event.getHand()).getTagCompound() == null || !event.getEntityPlayer().getHeldItem(event.getHand()).getTagCompound().hasKey("RegistryNameDomain"))
                    {
                        ...
                        MultiMob.LOGGER.debug(ItemSummonerOrb.getEntityNBT(entity));
                        ItemSummonerOrb.setEntityData(event.getEntityPlayer().getHeldItem(event.getHand()), entity, event.getEntityPlayer());
                    }
        ...
        {
            while (base.targetTasks.taskEntries.stream().anyMatch(taskEntry -> taskEntry.action != null))
            {
                base.targetTasks.taskEntries.stream().filter(taskEntry -> taskEntry.action != null).findFirst().ifPresent(taskEntry -> base.targetTasks.removeTask(taskEntry.action));
            }
        }
        ...
        {
            return entity instanceof EntityLiving && entity.hasCapability(SUMMONABLE_ENTITY_CAPABILITY, null);
        }

        public static boolean spiritCanBeCaptured(EntityLiving entity, ISummonableEntity summonable, DamageSource source)
        {
            return entity != null && summonable != null && !summonable.isSummonedEntity() && source != null && source.getTrueSource() != null && source.getTrueSource() instanceof EntityPlayer && (int) entity.getMaxHealth() >= 0 && entity.getEntityWorld().rand.nextInt((int) entity.getMaxHealth() + 1) == 0;
        }
    }
}